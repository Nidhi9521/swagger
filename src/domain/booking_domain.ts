...
class BookingDomain {
    ...
    async bookingCancel(req:Request,res:Response){
        try{

            var q:any=req.query;
            if(q.bookingId.length!=0){
                const bookingId:any=q.bookingId;
                await bookingmodel.updateOne({"_id":bookingId},{$set:{"status":"cancel"}});
                res.status(StatusCode.Sucess).send("Booking cancel Success")
                res.end();
            }else{
                res.status(StatusCode.Sucess).send("bookingId is wrong")
                res.end();
            }
    ...
    async userBookingHistory(req: Request, res: Response) {
        try {
            ...
            var bookingData = await bookingmodel.find({$match:[{ "user_id": uid },{"status":"sucess"}]});
            var hotelIdList: any = [];
